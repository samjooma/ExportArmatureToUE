...
class ExportToUEOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        rigify_armatures = [x for x in context.selected_objects if x.type == "ARMATURE" and any(child.type == "MESH" for child in x.children) and x.data.get("rig_id")]
        ...
        for created_armature in created_armatures:
            ...
            if created_armature.name.endswith(conversion_name_suffix):
                base_name = created_armature.name[:-len(conversion_name_suffix)]
            else:
                base_name = created_armature.name
        ...
        if self.export_armature_meshes:
            for created_armature in created_armatures:
                child_armature_meshes = [
                    x for x in created_armature.children if
                    x.type == "MESH" and
                    any(isinstance(y, bpy.types.ArmatureModifier) and y.object == created_armature for y in x.modifiers)
                ]
                ...
                if "Armature" in bpy.data.armatures:
                    ...
                    return {"Canceled"}
                created_armature.name = "Armature"
                created_armature.data.name = "Armature"
                ...
                for child_mesh in child_armature_meshes:
                    ...
                    context_override["selected_objects"] = [created_armature, child_mesh]
        ...
        if self.export_child_meshes:
            ...
            for created_armature in created_armatures:
                non_armature_child_meshes = non_armature_child_meshes.union({
                    x for x in created_armature.children if
                    x.type == "MESH" and x.parent_type == "BONE" and x.parent_bone in created_armature.data.bones
                })
